...
const hitboxShape: b2CircleShape = new b2CircleShape(toMeters(40));
const hitboxDef: b2FixtureDef = {
    shape: hitboxShape,
    density: 1.0,
    friction: 0.0,
    filter: {
        categoryBits: COLLISION_MASK.PLAYER,
        maskBits: COLLISION_MASK.ENVRIONMENT | COLLISION_MASK.VIEWBOX | COLLISION_MASK.FIRE | COLLISION_MASK.MOB,
    }
}

const viewportShape = getBoxShape(toMeters(1920 * 1.4 + 300), toMeters(1080 * 1.4 + 300))//: b2CircleShape = new b2CircleShape(toMeters(40));
...
export class PlayerPhysicsController extends PhysicsBodyController {
